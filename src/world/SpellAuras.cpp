...
pSpellAura SpellAuraHandler[TOTAL_SPELL_AURAS] =
{
    ...
    &Aura::ApplySpellMod,                                                   // 107 SPELL_AURA_ADD_FLAT_MODIFIER
    &Aura::ApplySpellMod,                                                   // 108 SPELL_AURA_ADD_PCT_MODIFIER
...
void Aura::SpellAuraModShapeshift(bool apply)
{
    ...
    else
    {
        ...
        if (m_target->IsCasting() && m_target->m_currentSpell && m_target->m_currentSpell->GetProto()
            && (m_target->m_currentSpell->GetProto()->Stances[0] & DecimalToMask(mod->m_miscValue)))
